  ...
  sections: [{
    ...
    children: [
      ...
      new Paragraph({
        ...
        children: [new TextRun({ text: `（2026年8月7日—8月13日）`, bold: true })]
      }),
      new Paragraph({
      ...
      new Paragraph({
        indent: { firstLine: 480 },
        children: [        new TextRun({ text: `截至8月13日，全省账外工程整改指标共 ` }),
            new TextRun({ text: `1122`, bold: true }),
            new TextRun({ text: ` 个（账外预告警站点 ` }),
            new TextRun({ text: `412`, bold: true }),
            new TextRun({ text: ` 个、疑似已账外且未立项站点 ` }),
            new TextRun({ text: `710`, bold: true }),
            new TextRun({ text: ` 个），较8月7日（整改指标 ` }),
            new TextRun({ text: `1528`, bold: true }),
            new TextRun({ text: ` 个）一周内压降 ` }),
            new TextRun({ text: `406`, bold: true }),
            new TextRun({ text: ` 个，降幅 ` }),
            new TextRun({ text: `26.6%`, bold: true }),
            new TextRun({ text: `。其中：` })]
      }),
      new Paragraph({
        numbering: { reference: "bullet-list-0", level: 0 },
        children: [        new TextRun({ text: `账外预告警站点：499 个 → 412 个，压降 ` }),
            new TextRun({ text: `87`, bold: true }),
            new TextRun({ text: ` 个（-17.4%）。` })]
      }),
      new Paragraph({
        numbering: { reference: "bullet-list-0", level: 0 },
        children: [        new TextRun({ text: `疑似已账外且未立项站点：1029 个 → 710 个，压降 ` }),
            new TextRun({ text: `319`, bold: true }),
            new TextRun({ text: ` 个（-31.0%）。` })]
      }),
      new Paragraph({
        indent: { firstLine: 480 },
      ...
      new Paragraph({
        indent: { firstLine: 480 },
        children: [new TextRun({ text: `整改成效较好的前三名：`, bold: true })]
      }),
      new Table({
      ...
      new Paragraph({
        indent: { firstLine: 480 },
        children: [new TextRun({ text: `整改推进较慢的后三名分公司：`, bold: true })]
      }),
      new Table({
        width: { size: 9025, type: WidthType.DXA },
        columnWidths: [1805, 1805, 1805, 1805, 1805],
        ...
        new TableRow({
          ...
          new TableCell({
            ...
            borders: cellBorders,
            verticalAlign: VerticalAlign.CENTER,
            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun(`143`)] })]
          }),
          new TableCell({
            width: { size: 1805, type: WidthType.DXA },
            borders: cellBorders,
      ...
      new Paragraph({
        indent: { firstLine: 480 },
        children: [new TextRun({ text: `各分公司整改指标变化明细`, bold: true })]
      }),
      new Table({
      ...
      new Paragraph({
        indent: { firstLine: 480 },
        children: [        new TextRun({ text: `FTTH/宽带接入`, bold: true }),
            new TextRun({ text: ` 为整改重点专业，整改指标占比超九成，是本周整改成效的主要来源。` })]
      }),
      new Table({
        width: { size: 9025, type: WidthType.DXA },
        columnWidths: [1805, 1805, 1805, 1805, 1805],
        ...
        new TableRow({
          ...
          new TableCell({
            ...
            borders: cellBorders,
            verticalAlign: VerticalAlign.CENTER,
            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `FTTH/宽带接入合计`, bold: true })] })]
          }),
          new TableCell({
            width: { size: 1805, type: WidthType.DXA },
            borders: cellBorders,
          ...
          new TableCell({
            ...
            borders: cellBorders,
            verticalAlign: VerticalAlign.CENTER,
            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `1313`, bold: true })] })]
          }),
          new TableCell({
            width: { size: 1805, type: WidthType.DXA },
            borders: cellBorders,
            verticalAlign: VerticalAlign.CENTER,
            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `909`, bold: true })] })]
          }),
          new TableCell({
            width: { size: 1805, type: WidthType.DXA },
            borders: cellBorders,
            verticalAlign: VerticalAlign.CENTER,
            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: `-404`, bold: true })] })]
          })
          ]
        })
        ]
      ...
      new Paragraph({
        indent: { firstLine: 480 },
        children: [        new TextRun({ text: `FTTH/宽带接入专业周压降 404 个，占全省总压降量（406 个）的 ` }),
            new TextRun({ text: `99.5%`, bold: true }),
            new TextRun({ text: `，其中疑似已账外且未立项站点压降 324 个，推进力度最大。政企专线、无线网、应急/批量工程等专业整改指标基数较小，需同步关注整改进度，防止账外问题向其他专业蔓延。` })]
      }),
      new Paragraph({
        heading: HeadingLevel.HEADING_2,
        outlineLevel: 1,
      ...
      new Paragraph({
        ...
        children: [        new TextRun({ text: `荆州、武汉、咸宁分公司`, bold: true }),
            new TextRun({ text: `要查明整改指标不降反升的原因，倒排工期，逐站销号，确保整改指标持续压降。` })]
      }),
      new Paragraph({
        numbering: { reference: "num-list-0", level: 0 },
        children: [new TextRun({ text: `已账外且已立项站点为既定事实，各分公司要举一反三，从源头规范"先立项、后施工"流程，杜绝新增账外工程。` })]
      }),
      new Paragraph({
        numbering: { reference: "num-list-0", level: 0 },
        children: [new TextRun({ text: `各分公司要对照剩余整改清单，于8月20日前完成整改销号，确保月底账外工程整改目标全面达成。` })]
      })
    ]
  }]
});

Packer.toBuffer(doc).then(buffer => fs.writeFileSync("D:/工作/周报/账外数据/账外工程整改周通报（8月7日-8月13日）.docx", buffer));
